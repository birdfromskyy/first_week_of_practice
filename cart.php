...
<section class="shopping-cart">
   ...
   <form action="" method="POST" class="box">
      ...
      <div class="sub-total"> Итого : <span><?= $sub_total = ($fetch_cart['price'] * $fetch_cart['quantity']); ?>₽</span> </div>
   </form>
   <?php
      $grand_total += $sub_total;
      }
   }else{
      echo '<p class="empty">Корзина пустая</p>';
   }
   ?>
   </div>

   <div class="cart-total">
      <p>Всего : <span><?= $grand_total; ?>₽</span></p>
      <a href="shop.php" class="option-btn">продолжить покупку</a>
      <a href="cart.php?delete_all" class="delete-btn <?= ($grand_total > 1)?'':'disabled'; ?>">удалить</a>
      <a href="checkout.php" class="btn <?= ($grand_total > 1)?'':'disabled'; ?>">купить</a>
   </div>
