...
if(isset($_POST['update_profile'])){
   ...
   $update_profile->execute([$name, $email, $kitchen_id]);
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image->execute([$image, $kitchen_id]);
         if($update_image){
   ...
   if(!empty($update_pass) AND !empty($new_pass) AND !empty($confirm_pass)){
      if($update_pass != $old_pass){
         ...
         $update_pass_query->execute([$confirm_pass, $kitchen_id]);
...
<section class="update-profile">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="flex-btn">
         ...
         <a href="kitchen_home.php" class="option-btn">вернуться назад</a>
      </div>
   </form>
